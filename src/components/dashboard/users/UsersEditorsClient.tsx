...
type Editor = { id: string; name: string; email: string; posts: number; courses: number; lastActive: string; permission: Permission; };

const MOCK: Editor[] = [
  { id: '1', name: 'Amaka Obi', email: 'amaka@nnamdiobi.com', posts: 14, courses: 2, lastActive: '2026-06-13', permission: 'full' },
  { id: '2', name: 'Chike Eze', email: 'chike@nnamdiobi.com', posts: 8, courses: 1, lastActive: '2026-06-11', permission: 'limited' },
  { id: '3', name: 'Fatima Bello', email: 'fatima@nnamdiobi.com', posts: 5, courses: 0, lastActive: '2026-06-09', permission: 'limited' },
];

const PERM: Record<Permission, string> = {
  full:    'text-blue-400 bg-blue-400/10 border-blue-400/20',
  limited: 'text-[#888] bg-white/5 border-white/10',
};

export function UsersEditorsClient() {
  return (
    <SectionLayout
      title="Editors"
      subtitle="Users with content creation and editing permissions"
      cta={{ label: 'Invite Editor', onClick: () => {} }}
      stats={[
        { label: 'Total Editors', value: MOCK.length },
        { label: 'Full Access', value: MOCK.filter((e) => e.permission === 'full').length },
        { label: 'Total Posts', value: MOCK.reduce((s, e) => s + e.posts, 0) },
        { label: 'Total Courses', value: MOCK.reduce((s, e) => s + e.courses, 0) },
      ]}
    >
        ...
        <table className="w-full">
          <thead>
            <tr style={{ backgroundColor: 'var(--adm-card)', borderBottom: '1px solid var(--adm-border)' }}>
              {['Name', 'Email', 'Posts', 'Courses', 'Last Active', 'Permission', ''].map((h) => (
          ...
          <tbody>
            {MOCK.map((e, i) => (
              <tr key={e.id} style={{ backgroundColor: 'var(--adm-card)', borderTop: i > 0 ? '1px solid var(--adm-border)' : undefined }}>
                ...
                <td className="px-4 py-3.5 text-sm" style={{ color: 'var(--adm-text)' }}>{e.courses}</td>
                ...
                <td className="px-4 py-3.5">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border capitalize ${PERM[e.permission]}`}>{e.permission}</span>
                </td>
                <td className="px-4 py-3.5">
