...
export function HomeRight() {
  return (
    <div className="bg-[#060606] flex flex-col justify-between p-10 gap-10 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pb-8 border-b border-[#1c1c1c]">
        {CONTACT_STRIP.map((item) => (
          <Link key={item.href} href={item.href} className="group flex flex-col gap-0.5">
            <span className="font-body text-[11px] text-[#888]">{item.label}</span>
            <span className="font-body text-[11px] text-[#f0ede8] group-hover:text-[#677db7] transition-colors">
              {item.action}
            </span>
          </Link>
        ))}
      ...
      {/* Bottom: bio + CTA */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 pt-8 border-t border-[#1c1c1c]">
        ...
        <Link
          href="/contact"
          className="shrink-0 bg-[#677db7] hover:bg-[#5a6da0] text-[#f0ede8] font-body text-[11px] tracking-[0.15em] uppercase px-6 py-3 rounded-xl transition-colors"
        >
          Work with me
        </Link>
      </div>
