...
export default async function MyReviewsPage() {
  ...
  if (!user) return <div className="p-8"><p style={{ color: 'var(--adm-muted)' }}>Please log in.</p></div>;
    ...
    <div className="p-8 max-w-2xl">
      ...
      {pending.length > 0 && (
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-3">
            {pending.map((r) => {
              const sub = r.assignment_submissions as unknown as { assignment_id: string; assignments: { title: string; courses: { title: string } | null } | null } | null;
              return (
                <Link key={r.id} href={`/home/reviews/${r.id}`}
                  ...
                  <div>
                    <p className="text-sm font-semibold" style={{ color: 'var(--adm-text)' }}>{sub?.assignments?.title ?? 'Assignment'}</p>
                    <p className="text-xs mt-0.5" style={{ color: 'var(--adm-muted)' }}>{sub?.assignments?.courses?.title}</p>
                  </div>
      ...
      {(!reviews || reviews.length === 0) && (
        <div className="flex flex-col items-center justify-center py-24 rounded-2xl border"
          ...
          <p className="text-xs mt-1" style={{ color: 'var(--adm-muted)' }}>Submit an assignment first to get assigned reviews.</p>
        </div>
      )}
    </div>
  );
